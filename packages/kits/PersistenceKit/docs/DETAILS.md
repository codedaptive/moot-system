...
`Storage.datasetStore` adds typed user tables.
`DatasetStore` supports create, append, query, stats, and drop.
SQLite, PostgreSQL, and in-memory backends implement it.
Column identifiers use one strict portable rule.
Text equality and order use UTF-8 bytes on every backend.
HLC comparison uses the full logical-clock order.
...
This file defines the user-dataset storage contract.
Each dataset owns a table derived from its UUID.
The schema can name one primary key and simple indexes.
Rows use `TypedValue`.
Queries reuse `StoragePredicate` and `OrderClause`.

Every user column name must match the portable identifier rule.
Invalid names fail the whole operation.
The backend never rewrites a bad name.
Column stats return count, distinct count, null count, minimum, and maximum.

SQLite, PostgreSQL, and in-memory stores share this contract.
Their text order is UTF-8 byte order.
This makes Swift and Rust results agree.
