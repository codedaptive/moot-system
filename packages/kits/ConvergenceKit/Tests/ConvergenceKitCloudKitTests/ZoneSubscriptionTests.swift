...
struct RemoteWakeIntegrationTests {
    ...
    func correctZoneEmitsRemoteWakeReceived() async throws {
        ...
        // Subscribe with attachment guaranteed BEFORE the notification fires.
        // Plain subscribe() attaches via an unstructured Task; under parallel
        // test load the emit raced the attachment, the event was missed, and
        // the unbounded for-await below hung the suite for hours (2026-07-17).
        let stream = await engine.subscribeAttached()

        // races a timeout child so a missed event fails fast instead of
        // hanging (a for-await deadline check only runs when an event arrives).
        let eventTask = Task<SyncEvent?, Never> {
            await withTaskGroup(of: SyncEvent?.self) { group in
                group.addTask {
